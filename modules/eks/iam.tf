...
# Cart Pod가 사용할 IAM 역할
resource "aws_iam_role" "retail-cart-dynamo-role" {
  name = "retail-cart-dynamo-role"
    ...
    Statement = [{
      ...
      Condition = {
        StringEquals = {
          ...
          "aws:RequestTag/kubernetes-namespace" = local.cart_namespace

          "aws:RequestTag/kubernetes-service-account" = local.cart_service_account
        }
      }
    }]
  })
